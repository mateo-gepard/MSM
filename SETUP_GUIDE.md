...
- `CRON_SECRET`: random server-only secret sent as `Authorization: Bearer <CRON_SECRET>` to the hourly booking-reconciliation route. Production requires at least 32 UTF-8 bytes and rejects template placeholders.
...
3. Deploy the application to staging with test provider/service secrets, staff MFA, both signed webhook routes, and the authenticated hourly reconciliation route. Publish disabled test-mode offers, compare every commercial fact with Stripe, register the callbacks/cron, and then enable only the reviewed staging offers and both staging gates.
...
6. Deploy production with live provider objects/secrets, signed endpoints, staff MFA, and hourly cron, while every new-sale control remains off. Publish the live offer versions disabled and compare them with Stripe and the approved public display.
