...
3. Deploy the application to staging with test Stripe/Cal.com webhook secrets and `CRON_SECRET`; register both signed callbacks and the authenticated hourly reconciliation schedule.
...
6. Deploy production with live signed callbacks and hourly reconciliation while all new-sales controls remain off. Publish and verify the live offer versions with their pointers still disabled.
